...
func TestMakeSRun(t *testing.T) {
	...
	for i := 1; i <= nt.Count; i++ {
		...
		expect := fmt.Sprintf("cd %s && hspice -hpp -mt 4 -i %s -o ./hspice &> ./hspice.log && wv -k -ace_no_gui ../../extract.ace &> wv.log && cat store.csv | sed '/^#/d;1,1d' | awk -F, '{print $2}' | xargs -n3 > ../Result/SEED%d.csv && cd ../ && rm -rf SEED*", dst, input, i)
