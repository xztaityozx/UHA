...
func TestAllSRun(t *testing.T) {
	...
	t.Run("001 tryMkRangeSEEDDstDir", func(t *testing.T) {
		...
		p := filepath.Join(rst.BaseDir, fmt.Sprintf("RangeSEED_Vtn%.4fVtp%.4f_Sigma%.4f_Monte%s/SEED%03d", rst.Vtn.Voltage, rst.Vtp.Voltage, rst.Sigma, rst.Monte, rst.SEED))
	...
	t.Run("003 writeRangeSEEDSPI", func(t *testing.T) {
		...
		f := filepath.Join(rst.Sim, "Vtn0.6000Vtp-0.6000Monte500_Sigma0.1000_SEED001.spi")
